...
class TDLearningModel(Model):
    ...
    def __init__(self, learning_rate=0.1, discount_factor=0.9, exploration_rate=0.2, 
        ...
        self.exploration_rate = exploration_rate      # Epsilon: For exploration-exploitation tradeoff
    ...
    def get_action(self, state, history):
        ...
        if self.previous_action is not None and self.tick_counter < self.min_action_duration:
        ...
        if state_key not in self.q_table:
            self.q_table[state_key] = {True: 0.0, False: 0.0}  # Initialize Q-values for both actions
...
def train_model(episodes=100, ticks_per_episode=36000, save_interval=10):
    ...
    for episode in range(1, episodes + 1):
        ...
        reward = simulate(model, TRAINING=True, TICKS_PER_SECOND=60, NO_OF_TICKS=ticks_per_episode)
    ...
    print(f"Average reward over all episodes: {sum(rewards_history) / len(rewards_history):.2f}")
...
def evaluate_model(model_path, num_evaluations=5, ticks_per_evaluation=36000):
    ...
    for i in range(num_evaluations):
        ...
        reward = simulate(model, TRAINING=True, TICKS_PER_SECOND=60, NO_OF_TICKS=ticks_per_evaluation)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='TD Learning for Traffic Signal Control')
    parser.add_argument('--mode', type=str, default='train', choices=['train', 'evaluate', 'run'],
                        help='Mode: train, evaluate, or run')
    parser.add_argument('--model', type=str, help='Path to model file (for evaluate and run modes)')
    parser.add_argument('--episodes', type=int, default=50, help='Number of training episodes')
    parser.add_argument('--ticks', type=int, default=36000, help='Number of ticks per episode')
    parser.add_argument('--save_interval', type=int, default=10, help='Save interval in episodes')
    
    args = parser.parse_args()
    
    if args.mode == 'train':
        train_model(episodes=args.episodes, ticks_per_episode=args.ticks, save_interval=args.save_interval)
